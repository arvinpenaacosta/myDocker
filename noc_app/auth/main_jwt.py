...
# Header file path
PAGE_HEADER = os.getenv("PAGE_HEADER", "Arvin Inventory Management")  # Default to 'Page Footer' if not specified
...
async def dashboard(request: Request, username: str = Depends(get_current_user)):
    return templates.TemplateResponse("restricted/dashboard.html", {"request": request, "username": username, "pageheader": PAGE_HEADER, "pagefooter": PAGE_FOOTER})
...
async def read_items(request: Request, username: str = Depends(get_current_user)):
    return templates.TemplateResponse("restricted/master_items.html", {"request": request, "username": username, "pageheader": PAGE_HEADER, "pagefooter": PAGE_FOOTER})
